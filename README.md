...
![image](https://github.com/Glibusss/Gleb_Makarenko_Spring2024_Highload/assets/113942267/70cff85d-5162-4aac-8f48-28c82ecede67)
...
Для обработки отказов определенных компонентов будет использоваться Graceful Degradation. Для обеспечения Graceful Degradation на клиенте будет использоваться локальный кеш и SQLite
...
* Backend - будует использоваться уменьшение посылаемых запросов на проблемный хост и circuit breaker. В случае слишком большого числа запросов мы либо игнорирем запросы(в случае если с одного IP идёт слишком много), либо намерено замедляем их обработку(если слишком большое количество запросов с разных IP адресов).
...
Сервис | Хостинг| Конфиг| Count| Покупка($) шт.|
------ |---------- |---------- |--- |--- |------------ |
nginx  | У себя    | 16 core CPU; 4x8 GB RAM 2400 MHz DDR4 | 9 | 1 795,56|
Auth   | У себя    | 12 core CPU; 1х4 GB RAM 2400 MHz DDR4 | 2 | 1 221,76|
Users  | У себя    | 4 core CPU; 1х4 GB RAM 2400 MHz DDR4 | 1 | 924,75|
Channels  | У себя    | 2 core CPU; 1х4 GB RAM 2400 MHz DDR4 | 1 | 811,10|
Posts  | У себя    | 12 core CPU; 1х4 GB RAM 2400 MHz DDR4 | 2 | 1 221,76|
Comments  | У себя    | 16 core CPU; 1х4 GB RAM 2400 MHz DDR4 | 132 | 4 088,8|
Messenger | У себя | 16 core CPU; 1х4 GB RAM 2400 MHz DDR4 | 264 | 4 088,8|
...
Ежедневный онлайн составляет около 55 000 000 пользователей. Если учесть, что все они будут одновременно пользоваться нашим сервисом, то потребуется token(64 байта) + id (4 байта) + user_id (4 байта) = 72 байта на одного пользователя. С учетом коэфициента запаса 2 на дополнительные поля, которые могут возкникнуть в нашей структуре, возьме объем в 216 байт Тогда для 55 200 000: 11.10 ГБ. Для более эффективной обработки и повышения надежности можно сделать 1 master и 2 slave сервера с репликами. Всего 6 серверов.
