...
![image](https://github.com/Glibusss/Gleb_Makarenko_Spring2024_Highload/assets/113942267/1f47b7ff-2634-478c-adcc-d5d07498f67a)

![image](https://github.com/Glibusss/Gleb_Makarenko_Spring2024_Highload/assets/113942267/a831aa7c-f1e9-413c-ace0-4792c0f2dc90)
...
* Сервис Upload Manager будет отвечать за обработку, загрузку и очистку файлов в S3.
...
* Clean Manager отвечает за периодическую очистку/архивацию файлов(То есть периодически смотрит хранилище . Очистка файлов происходит если в течение 30 дней с момента отправки файл не был прочитан, архивация происходит в случае, если пользователь удалил сообщение с данным файлом или файл не читался хотя бы 30 дней. Все эти данные можно получить с помощью API Amazon S3.
...
* При слишком большой нагрузке на мессенджер/посты/комментарии происходит пересылка сообщений сначала с файлами худшего качества, после - без файлов. Такую логику можно реализовать например за счёт ngx_http_upload_module
* При слишком большой нагрузке на мессенджер, отключаются стикеры
* При слишком большой нагрузке на мессенджер/посты/комментарии отключаются реакции
