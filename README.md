...
Будет использоваться API Gateway для обеспечения изолированности логики различных клиентов и централизованной точки маршрутизации. Также рахитектура будет разделена на микросервисы для изоляции бизнес-логики.
...
* Backend - будует использоваться уменьшение посылаемых запросов на проблемный хост и circuit breaker. В случае слишком большого числа запросов мы либо игнорирем запросы(в случае если с одного IP идёт слишком много), либо намерено замедляем их обработку(если слишком большое количество запросов с разных IP адресов). Также в случае отказа БД, backend будет кешировать запросы на запись, которые после восстановления будут отправлены в БД.
