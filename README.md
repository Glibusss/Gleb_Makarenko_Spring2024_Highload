...
![Highload Gleb Makarenko DB (5)](https://github.com/Glibusss/Gleb_Makarenko_Spring2024_Highload/assets/113942267/714da8ba-943d-49db-8ed4-c16d9e38ae8b)
...
* ```post```
   * ```content``` - частичное совпадение, допускаются различные формы слов
   * ```channels.name``` по полю ```channel_id``` - частичное совпадение

* ```channel```
   * ```name``` - частичное совпадение, допускаются различные формы слов

Поиск по сообщениям и постам будет отдавать приоритет самым последним

Поиск по номеру без приоритезации т.к. мы используем полное совпадение по номеру

Поиск канала будет отдавать приоритет каналам с наибольшим числом подписчиков
