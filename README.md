...
Т.к. в качестве ОС для серверов используется Linux, то для обеспечаения отказоустойчивости системы стоит использовать framework *keepalived* благодаря следующим преимуществам:
* Производительность
* Открытость
* Простота настройки
...
* Пробы доступности (Liveness Probes). Благодаря ним можно убедиться, что узел находится в рабочем состоянии
* Пробы готовности (Readiness Probes). Благодаря ним можно убедиться, что сервер готов принимать трафик
* Пробы восстановления после сбоя (Failure Recovery Probes). В случае сбоя можно проверить, способен ли сервер восстановиться

L4:
На этом уровне можно использовать NGINX благодаря следующим его преимуществам:
* Открытость
* Гибкость
* Производительность

L7:
На этом уровне для балансировщика нагрузки важна поддержка gRPC. Будем использовать балансировщик Envoy Proxy благодаря следующим преимуществам:
